...
## To-Do List
- [ ] 페어 매칭 기능
  - [x] 파일로부터 페어 이름 올바르게 불러온다.
  - [x] 페어를 두명씩 매칭하며, 홀수인 경우 마지막 페어는 3명으로 구성한다.
    - [x] 매칭은 `shuffle`을 통해 `List`를 섞은 후 순서대로 매칭한다.
  - [x] 같은 레벨에서 이미 페어를 맺은 크루와는 다시 페어로 매칭될 수 없다.
  - [x] 매칭을 했는데 다른 미션과 중복된 페어 매칭이 존재하면 재매칭을 한다.
    - [x] 재매칭을 할 때는 안내 문구를 출력 후 매칭을 진행한다.
  - [ ] 이미 매칭이 존재하는 경우 재매칭 여부를 선택할 수 있다.
    - [ ] `아니오`를 택할 경우 코스, 레벨, 미션을 다시 선택한다.
- [x] 페어 조회 기능
  - [x] 입력값에 따른 페어 정보를 출력한다.
  - [ ] 페어 조회 시 매칭 이력이 없으면 아래의 메시지를 출력한다.
    - `[ERROR] 매칭 이력이 없습니다.`
